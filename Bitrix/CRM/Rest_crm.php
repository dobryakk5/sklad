...
'crm.deal.alfasklad.list' => array(
    ...
    public static function updateContract($query, $n, \CRestServer $server){
        $path = $_SERVER['DOCUMENT_ROOT'].'/logs/'.date('Y').'/'. strtolower(date('F')).'/'.date('d');
        if(!is_dir($path)){
            mkdir($path, 0777, true);
        }

        file_put_contents($path . "/updateDocumentsFrom1C.txt",print_r($query,true), FILE_APPEND);
        $updatedId = [];
        $data = [];
    ...
    public static function getCompany($query, $n, \CRestServer $server)
    {
        ...
        while ($item = $res->Fetch()) {
            ...
            if( $data["REQUSITES"]["PRESET_ID"] == 3 && ! empty($data['PASSPORT_SERIES']) ) {

                Helper::setValueUserField('CRM_COMPANY', $item['ID'], 'UF_SENT_TO_1C', true);
                $companies[] = $data;

            }  elseif ($data["REQUSITES"]["PRESET_ID"] != 3 && !empty($data['INN'])) {

                Helper::setValueUserField('CRM_COMPANY', $item['ID'], 'UF_SENT_TO_1C', true);
                $companies[] = $data;

            } else {
                //file_put_contents(__DIR__ . '/log_error.txt', date(DATE_ATOM)."\n", FILE_APPEND);
               //file_put_contents(__DIR__ . '/log_error.txt', print_r($data, true), FILE_APPEND);
            }


        }
        //file_put_contents(__DIR__ . '/log2.txt', date(DATE_ATOM)."\n", FILE_APPEND);
        //file_put_contents(__DIR__ . '/log2.txt', print_r($companies, true), FILE_APPEND);
    ...
    public static function updateCompany($query, $n, \CRestServer $server)
    {
        ...
        file_put_contents($path . "/updateCompanyesFrom1C.txt",print_r($query,true), FILE_APPEND);
        $companies = [];
     ...
     public static function updateOrderDeal($query, $n, \CRestServer $server){
        ...
        //лог сделок обновленных из 1с
       file_put_contents($path . "/updateDealsFrom1C.txt",print_r($query,true), FILE_APPEND);
        /*$Deals = [];
...
//                file_put_contents(__DIR__ . '/deals.txt', date(DATE_ATOM)."\n", FILE_APPEND);
//                file_put_contents(__DIR__ . '/deals.txt', print_r($dataCompany, true), FILE_APPEND);
//                file_put_contents(__DIR__ . '/deals.txt', print_r($deal, true), FILE_APPEND);

                if( $dataCompany["REQUSITES"]["PRESET_ID"] == 3 &&  empty($dataCompany['PASSPORT_SERIES']) ) {
                    continue;

                }  elseif ($dataCompany["REQUSITES"]["PRESET_ID"] != 3 && empty($dataCompany['INN'])) {
                    continue;
        ...
        file_put_contents($path . "/getDealsFor1C.txt",print_r($Deals,true), FILE_APPEND);
   ...
   switch ($arFields['UF_CRM_1566814003549'])
     ...
     public function SendContractToSite($id_contract)
    {
        ...
        if($contract){
            ...
            file_put_contents($path . "/1_SendContractToSite.txt",print_r($param,true), FILE_APPEND);
            $resutlOrderSend = HelperRest::SendRequest($param, $url);
            $resutlOrderSend = json_decode($resutlOrderSend,true);
    ...
    public static function OnAfterCrmInvoiceAddHandler($arFields)
    {
        ...
        if($invoice){
            ...
            file_put_contents($path . "/3_SendInvoiceToSite.txt",print_r($resutlOrderSend,true), FILE_APPEND);
            $resutlOrderSend = json_decode($resutlOrderSend,true);
