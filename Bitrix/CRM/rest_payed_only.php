...
class Rest
{
    ...
    static function NewOrderSendToB24(\Bitrix\Main\Event $event)
    {
        ...
        // ИЗМЕНЕНИЕ: определяем, есть ли у заказа платежная система 7 (ЮKassa)
        $isYooKassaPayment = in_array(7, array_map('intval', (array)$paymentIds), true);
 ...
 */
        ...
        if ($isNew) {
            ...
            $data['ORDER'] = $orderArr;
                        ...
                        if(!$isOrderFromCrm) {
                                // ИЗМЕНЕНИЕ:
                                // если способ оплаты = 7 (ЮKassa), то addDealFromSite не вызываем
                                if(!$isYooKassaPayment) {
                                        ...
                                        Helper::log('/logs/rest.txt', "Заказ не отправлен в Б24 через addDealFromSite, т.к. у заказа способ оплаты = 7 (ЮKassa)");
                                }
