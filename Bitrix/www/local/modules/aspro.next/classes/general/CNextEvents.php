...
class CNextEvents{
							...
							if($cntActive){
								...
								foreach($arSitesLids as $siteId){
									...
									if($arSite){
										...
										// catalog page
										$catalogPage = trim(CNext::GetFrontParametrValue("CATALOG_PAGE_URL", $siteId, false));
	...
	static function OnEndBufferContentHandler(&$content)
	{
		...
		{
			global $SECTION_BNR_CONTENT, $arRegion, $APPLICATION, $bShowHeaderSimple;
			...
			if($GLOBALS['_USE_LAZY_LOAD_']){
				...
				$tmpContent = preg_replace('/<img ((?![^>]*\bdata-src\b)[^>]*>)/i'.BX_UTF_PCRE_MODIFIER, '<img lazyload ${1}', $content);
				if(isset($tmpContent)){
					$content = $tmpContent;
					$content = preg_replace('/(<img lazyload [^>]*)src=/i'.BX_UTF_PCRE_MODIFIER, '${1}src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src=', $content);
					$content = preg_replace('/(<img lazyload .*?)class=([\'"])/i'.BX_UTF_PCRE_MODIFIER, '${1}class=${2}lazyload ', $content);
					$content = preg_replace('/<img lazyload (?![^>]*\bclass\b)/i'.BX_UTF_PCRE_MODIFIER, '${0}class="lazyload" ', $content);
				}
				else{
					// preg error
				}
