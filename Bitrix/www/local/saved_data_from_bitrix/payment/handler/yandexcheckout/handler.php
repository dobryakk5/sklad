    ...
    PaySystem\Cashbox\IFiscalizationAware
{
    ...
    private function createYandexPayment(Payment $payment, Request $request, bool $isRepeated = false): PaySystem\ServiceResult
    {
        ...
        if ($params["receipt"]["customer"]['email'] == 'info@alfasklad.ru' || empty($params["receipt"]["customer"]['email'])) {
            if (!empty($_REQUEST["ORDER_ID"])) {
                ...
                $params["receipt"]["customer"]['email']=$rwUser['EMAIL'];
            }
        }

        foreach ($params['receipt']['items'] as $key => $item) {
            //if ($item['description'] != "Гарантийный депозит") {
            $params['receipt']['items'][$key]['vat_code'] = 4;
            $params['receipt']['items'][$key]['description'] = 'Аренда бокcа';
