			...
			<?=$arResult["FORM_HEADER"]?>
				...
				<div class="form-body">
					...
					<?if(is_array($arResult["QUESTIONS"])):?>
						...
						<?foreach( $arResult["QUESTIONS"] as $FIELD_SID => $arQuestion ){ //echo "<pre>"; print_r($arQuestion); echo "</pre>";?>
							<?if( $arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden' ){
								echo $arQuestion["HTML_CODE"];
							}else{?>
							<?if($arQuestion['FIELD_TYPE'] != "file" && $FIELD_SID != 'RATING' && $FIELD_SID != "HIDE_NAME" && $FIELD_SID != "MAINPAGE"):?>
								<div  data-SID="<?=$FIELD_SID?>">
										<div class="form-group <?=( $arQuestion['FIELD_TYPE'] != "file" ? "animated-labels" : "");?> <?=( $arQuestion['VALUE'] ? "input-filed" : "");?><?=( $arQuestion['FIELD_TYPE'] == 'list' ? "input-filed" : "");?>">
											<?/*str_replace('for="', 'class="input-label" for="POPUP_', $arQuestion["CAPTION"]);*/?>
											
											<label class="input-label">
												<div class="input-border">
													<?$arQuestion["HTML_CODE"] = str_replace('id="', 'placeholder="'.$arQuestion["NAME"].'" id="POPUP_', $arQuestion["HTML_CODE"])?>
													<?$arQuestion["HTML_CODE"] = str_replace('form-control', 'input-field', $arQuestion["HTML_CODE"])?>
													<?if($FIELD_SID != 'RATING'):?>
				...
				<div class="add-input-wrapper">
                    <div class="type-file">
                        <div class="link-input innerInputs" id="innerInput-1">
							...
							<?$arQuestion["HTML_CODE"] = str_replace('inputfile', 'input-files input-field', $arQuestion["HTML_CODE"])?>
							<?=$arQuestion["HTML_CODE"]?>						
							<label class="field__file-wrapper">
				...
				<?foreach( $arResult["QUESTIONS"] as $FIELD_SID => $arQuestion ){?>
					<?if($FIELD_SID == 'RATING'):?>
						...
						<div class="rating_wrap clearfix">
							...
							<?=str_replace('type="number"', 'type="hidden"', $arQuestion["HTML_CODE"])?>
						</div>
				...
				<?}?>
					<?if($arParams["SHOW_LICENCE"] == "Y"):?>
						...
						<label class="type-checkbox"><input class="input-field" type="checkbox"
                                                                         oninput="this.className = ''"
                                                                         value="Соглашение" required
																		 <?=(COption::GetOptionString("aspro.priority", "LICENCE_CHECKED", "N") == "Y" ? "checked" : "");?> 
																		 name="licenses_popup" value="Y"><span
...
<script>
	...
	$(document).ready(function(){
		...
		$('.popup form[name="<?=$arResult["IBLOCK_CODE"]?>"]').validate({
			...
			errorPlacement: function( error, element ){
				if($(element).hasClass('captcha')){
					...
					$(element).closest('.licence_block').before(error);
				}
        ...
        <? if($arParams["IBLOCK_ID"] == 62): ?>
		    $('input[type=file]').uniform({fileButtonHtml: BX.message('JS_FILE_BUTTON_NAME'), fileDefaultHtml: BX.message('JS_FILE_DEFAULT_STORAGE')});
        <? else: ?>
		...
		$('.form .add_text').on('click', function(){
			...
			$(this).closest('.form-group').find('.input-border').append('<input type="text" id="POPUP_'+name+'" name="'+name+'['+index+']"  class="input-field " value="" />');
		});
