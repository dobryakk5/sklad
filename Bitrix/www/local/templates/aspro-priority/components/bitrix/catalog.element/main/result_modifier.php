...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	$postfix = '';
	global $arSite;
	if(\Bitrix\Main\Config\Option::get("aspro.priority", "HIDE_SITE_NAME_TITLE", "N")=="N")
...
$arResult["SERVICES"]=CNextCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCache::GetIBlockCacheTag(CNextCache::$arIBlocks[SITE_ID]["aspro_priority_content"]["aspro_priority_services"][0]), "GROUP" => "ID")), array("IBLOCK_ID" => CNextCache::$arIBlocks[SITE_ID]["aspro_priority_content"]["aspro_priority_services"][0], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "PROPERTY_LINK_GOODS" => $arResult["ID"]), false, false, $arSelect);
...
$arServices = CNextCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCache::GetIBlockCacheTag(CNextCache::$arIBlocks[SITE_ID]["aspro_priority_content"]["aspro_priority_services"][0]), "GROUP" => "ID")), array("IBLOCK_ID" => CNextCache::$arIBlocks[SITE_ID]["aspro_priority_content"]["aspro_priority_services"][0], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y"), false, false, array_merge($arSelect, array("PROPERTY_LINK_GOODS_FILTER", "PROPERTY_LINK_GOODS")));
...
{
    if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_IFRAME2", "CML2_ARTICLE","TIZERS","LINK_REGION")) && in_array($arProp["CODE"],$arParams['PROPERTY_CODE']))
...
if($arResult["DISPLAY_PROPERTIES"])
{
	...
	{
		if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO2", "VIDEO_IFRAME2", "CML2_ARTICLE")))
