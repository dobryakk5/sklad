						...
						<ul>
							<?foreach($arResult["MORE_PHOTO"] as $i => $arImage){
								...
								<li id="photo-<?=$i?>" <?=(!$i ? 'class="current"' : '')?>>
									<?if(!$isEmpty){?>
										<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox-group="item_slider"')?> class="<?=($viewImgType=="POPUP" ? "popup_link fancybox" : "line_link");?>" title="<?=$title;?>">
					...
					<div class="wrapp_thumbs xzoom-thumbs">
						<div class="thumbs flexslider" data-plugin-options='{"animation": "slide", "selector": ".slides_block > li", "directionNav": true, "itemMargin":10, "itemWidth": 54, "controlsContainer": ".thumbs_navigation", "controlNav" :false, "animationLoop": true, "slideshow": false}' style="max-width:<?=ceil(((count($arResult['MORE_PHOTO']) <= 4 ? count($arResult['MORE_PHOTO']) : 4) * 64) - 10)?>px;">
							<ul class="slides_block" id="thumbs">
								<?foreach($arResult["MORE_PHOTO"]as $i => $arImage):?>
									<li <?=(!$i ? 'class="current"' : '')?> data-href="#photo-<?=$i?>" data-big_img="<?=$arImage["BIG"]["src"]?>" data-small_img="<?=$arImage["SMALL"]["src"]?>">
		...
		<div class="info_item">
				...
				<div class="top_info">
					...
					<?if(strlen($arResult["PREVIEW_TEXT"])):?>
						...
						<?if(strlen($arResult["DETAIL_TEXT"])):?>
							<div class="more_block icons_fa color_link"><span><?=\Bitrix\Main\Config\Option::get('aspro.priority', "EXPRESSION_READ_MORE_OFFERS_DEFAULT", GetMessage("MORE_TEXT_BOTTOM"));?></span></div>
						<?endif;?>
					<?endif;?>
				</div>
			<?}?>
			...
			<?$frame = $this->createFrame()->begin();?>
				...
				<div class="buy_block">
					...
					<?if(!$arResult["OFFERS"]):?>
						...
						<div class="counter_wrapp">
							<?if(($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]):?>
								<div class="counter pull-left big_basket" data-offers="<?=($arResult["OFFERS"] ? "Y" : "N");?>" data-item="<?=$arResult["ID"];?>" <?=(($arResult["OFFERS"] && $arParams["TYPE_SKU"]=="N") ? "style='display: none;'" : "");?>>
									 <div class="wrap">
												<span class="minus ctrl" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_DOWN']; ?>">
													<svg width="11" height="1" viewBox="0 0 11 1"><rect width="11" height="1" rx="0.5" ry="0.5"></rect></svg>
												</span>
                                        <div class="input"><input type="text" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<?=$arAddToBasketData["MIN_QUANTITY_BUY"]?>" class="count"></div>
                                        <span class="plus ctrl" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_UP']; ?>" <?=($arAddToBasketData["MAX_QUANTITY_BUY"] ? "data-max='".$arAddToBasketData["MAX_QUANTITY_BUY"]."'" : "")?>>
						...
						<?if($arAddToBasketData["ACTION"] !== "NOTHING"):?>
							<?if($arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"]!="N"):?>
								<div class="wrapp_one_click">
									<span class="btn white btn-lg type_block transition_bg one_click btn-transparent" data-item="<?=$arResult["ID"]?>" data-iblockID="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arResult["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)" data-event="jqm" data-name="one_click_buy">
										<span><?=GetMessage('ONE_CLICK_BUY')?></span>
									</span>
								</div>
							<?endif;?>
						<?endif;?>
                        <div class="wrapp_one_click order basketTrue">
                            <div class="wrapper">
                                <span class="btn btn-default wide-block animate-load btn-transparent" data-event="jqm" data-param-id="<?=CPriority::getFormID("aspro_priority_question");?>" data-autoload-need_product="<?=CPriority::formatJsName($arResult['NAME'])?>" data-name="question"><span><?=(strlen($arParams['ASK_TAB']) ? $arParams['ASK_TAB'] : GetMessage('ASK_TAB'))?></span></span>
                            </div>
                        </div>
					<?elseif($arResult["OFFERS"] && $arParams['TYPE_SKU'] == 'TYPE_1'):?>
					...
					<?elseif($arResult["OFFERS"] && $arParams['TYPE_SKU'] != 'TYPE_1'):?>
						<a href="#prices_offer" class="btn btn-default btn-lg slide_offer transition_bg type_block"><i></i><span><?=\Bitrix\Main\Config\Option::get("aspro.priority", "EXPRESSION_READ_MORE_OFFERS_DEFAULT", GetMessage("MORE_TEXT_BOTTOM"));?></span></a>
					<?endif;?>
				</div>
			<?$frame->end();?>
			</div>
...
//    print_r($arParams); die();
	...
	if($arResult["DISPLAY_PROPERTIES"]){
		foreach($arResult["DISPLAY_PROPERTIES"] as $arProp){
			if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_IFRAME", "CML2_ARTICLE"))){
	...
	if(isset($arResult["DISPLAY_PROPERTIES"]["VIDEO_IFRAME"]["VALUE"])){
		if(is_array($arResult["DISPLAY_PROPERTIES"]["VIDEO_IFRAME"]["VALUE"])){
			$arVideo = $arVideo + $arResult["DISPLAY_PROPERTIES"]["VIDEO_IFRAME"]["~VALUE"];
		}
		elseif(strlen($arResult["DISPLAY_PROPERTIES"]["VIDEO_IFRAME"]["VALUE"])){
			$arVideo[] = $arResult["DISPLAY_PROPERTIES"]["VIDEO_IFRAME"]["~VALUE"];
		}
	}
	if(strlen($arResult["SECTION_FULL"]["UF_VIDEO"])){
		$arVideo[] = $arResult["SECTION_FULL"]["~UF_VIDEO"];
	}
	if(strlen($arResult["SECTION_FULL"]["UF_VIDEO_IFRAME"])){
		$arVideo[] = $arResult["SECTION_FULL"]["~UF_VIDEO_IFRAME"];
	}
	...
	<div class="tabs">
		<ul class="nav nav-tabs">
			...
			<?if($arResult["DETAIL_TEXT"] ||
                ...
                (is_countable($arResult["SERVICES"]) && count($arResult["SERVICES"])) ||
                ( is_array($arResult["PROPERTIES"][$instr_prop]["VALUE"]) && count($arResult["PROPERTIES"][$instr_prop]["VALUE"]) ) ||
                    ...
                    ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):
                ?>
...
<!--					<div class="title-tab-heading visible-xs">--><?//=($arParams["TAB_OFFERS_NAME"] ? $arParams["TAB_OFFERS_NAME"] : GetMessage("OFFER_PRICES"));?><!--</div>-->
					...
					<table class="offers_table table">
						<thead>
							<tr>
								...
								<?endif;?>
								<?if($showSkUName):?>
									<td class="property names"><?=GetMessage("CATALOG_OFFER_NAME")?></td>
								<?endif;?>
                                                ...
                                                <?foreach( $arResult["SKU_PROPERTIES"] as $arProp ){?>
                                                    <?if(!$arProp["IS_EMPTY"]):?>
                                                        ...
                                                        <div class="property">
                                                            ...
                                                            <?if($arResult["TMP_OFFERS_PROP"][$arProp["CODE"]]){
                                                                if(!empty($arResult["TMP_OFFERS_PROP"][$arProp["CODE"]]["VALUES"][$arSKU["TREE"]["PROP_".$arProp["ID"]]]["NAME"]))
                                                                    echo "<b>{$arProp["NAME"]}: </b>" . $arResult["TMP_OFFERS_PROP"][$arProp["CODE"]]["VALUES"][$arSKU["TREE"]["PROP_".$arProp["ID"]]]["NAME"];?>
                                                            <?}else{
                                                                if (is_array($arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"])){
                                                                    if(!empty($arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"]))
                                                                        echo "<b>{$arProp["NAME"]}: </b>" . implode("/", $arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"]);
                                                                }else{
                                                                    if($arSKU["PROPERTIES"][$arProp["CODE"]]["USER_TYPE"]=="directory" && isset($arSKU["PROPERTIES"][$arProp["CODE"]]["USER_TYPE_SETTINGS"]["TABLE_NAME"])){
                                                                        ...
                                                                        if ($arData = $rsData->fetch()){
                                                                            ...
                                                                            if(isset($arValue["UF_NAME"]) && $arValue["UF_NAME"]){
                                                                                echo "<b>{$arProp["NAME"]}: </b>" . $arValue["UF_NAME"];
                                                                            }else{
                                                                                if(!empty($arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"]))
                                                                                    echo "<b>{$arProp["NAME"]}: </b>" . $arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"];
                                                                            }
                                                                        }
                                                                    }else{
                                                                        if(!empty($arSKU["PROPERTIES"][$arProp["CODE"]]["VALUE"]))
										...
										<!--noindex-->
											...
											<?if($arskuAddToBasketData["ACTION"] == "ADD"):?>
												<?if($arskuAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && !count($arSKU["OFFERS"]) && $arskuAddToBasketData["ACTION"] == "ADD" && $arskuAddToBasketData["CAN_BUY"]):?>
													<td class="counter_wrapp counter_block_wr">
                                                        <div class="counter  counter_block" data-item="<?=$arSKU["ID"];?>">
                                                            <span class="minus ctrl">
                                                                <svg width="11" height="1" viewBox="0 0 11 1"><rect width="11" height="1" rx="0.5" ry="0.5"></rect></svg>
                                                            </span>
                                                            <input type="text"  name="quantity" value="<?=$arskuAddToBasketData["MIN_QUANTITY_BUY"];?>" class="count">
											...
											<?if($arskuAddToBasketData["ACTION"] == "ADD" && $arskuAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"]!="N"):?>
												<td class="one_click_buy">
													<?$collspan++;?>
													<span class="btn white btn-lg type_block transition_bg one_click btn-transparent" data-item="<?=$arSKU["ID"]?>" data-offers="Y" data-iblockid="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arskuAddToBasketData["MIN_QUANTITY_BUY"];?>" data-props="<?=$arOfferProps?>" onclick="oneClickBuy('<?=$arSKU["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)" data-event="jqm" data-name="one_click_buy">
														<span><?=GetMessage('ONE_CLICK_BUY')?></span>
													</span>
												</td>
											<?endif;?>
			...
			<?if($arResult["DETAIL_TEXT"] ||
                (is_countable($arResult["SERVICES"]) &&count($arResult["SERVICES"])) ||
                (is_array($arResult["PROPERTIES"][$instr_prop]["VALUE"]) && count($arResult["PROPERTIES"][$instr_prop]["VALUE"]) ) ||
                ($arResult['ADDITIONAL_GALLERY'] || (is_countable($arResult["SECTION_FULL"]["UF_FILES"]) && count($arResult["SECTION_FULL"]["UF_FILES"])) ) ||
                ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):
                ?>
						...
						<?if($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB"):?>
							<div class="wraps">
								...
								<?else:?>
									...
									<?else:?>
                                        <?//echo "<pre>"; print_r($arResult["DISPLAY_PROPERTIES"]); die();?>
										<div class="char_block table-responsive">
													...
													<?if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_IFRAME", "CML2_ARTICLE"))):?>
														<?if((!is_array($arProp["VALUE"]) && strlen($arProp["VALUE"])) || (is_array($arProp["VALUE"]) && implode('', $arProp["VALUE"]))):?>
															<tr itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue">
																...
																<td class="char_value">
																	<span itemprop="value">
																		<?if(is_countable($arProp["VALUE"]) && count($arProp["VALUE"]) > 1):?>
...
<script type="text/javascript">
	BX.message({
		QUANTITY_AVAILIABLE: '<? echo COption::GetOptionString("aspro.priority", "EXPRESSION_FOR_EXISTS", GetMessage("EXPRESSION_FOR_EXISTS_DEFAULT"), SITE_ID); ?>',
		QUANTITY_NOT_AVAILIABLE: '<? echo COption::GetOptionString("aspro.priority", "EXPRESSION_FOR_NOTEXISTS", GetMessage("EXPRESSION_FOR_NOTEXISTS"), SITE_ID); ?>',
