...
<script type="text/javascript">
    ...
    function init(){ 
        ...
        <?foreach($arResult["MAP_POINTS"] as $key=>$arPoint) {?>
            ...
            myPlacemark_<?=$key?> = new ymaps.Placemark([<?=$arPoint["UF_MAP"]?>], {
                ...
                balloonContent: '<div><img src="<?=SITE_TEMPLATE_PATH?>/images/custom/map-logo-w.png" height="16px" style="margin-top: -5px;" /> <span style="font-size: 16px; font-weight: bold;"><?=$arPoint["NAME"]?></span></div>'+
                                ...
                                '<?if(strlen($arPoint["UF_PHONE"])>0 && false) { $phone = preg_replace('/[^\d+]/', '', $arPoint["UF_PHONE"]);?><div>Телефон: <a href="tel:<?=$phone?>" class="dark-color"><?=$arPoint["UF_PHONE"]?></a></div><?}?>'+
								'<a class="btn btn-default btn-xs" href="/rental_catalog/<?=$arPoint["CODE"]?>/" style="margin-top:10px;">Страница склада</a>',
        ...
        <?if( is_countable($arResult["MAP_POINTS"]) && count($arResult["MAP_POINTS"]) == 1):?>
