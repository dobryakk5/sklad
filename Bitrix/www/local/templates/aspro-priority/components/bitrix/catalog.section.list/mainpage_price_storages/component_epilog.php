...
<script type="text/javascript">
	...
	function init(){ 
		...
		<?foreach($arResult["SECTIONS"] as $key=>$arPoint) {?>
			...
			myPlacemark_<?=$key?>.events.add(['balloonopen'], function (e) {				
				//console.log(e.get('target').options.get('skladId'));
				$('.mainpage_price_storages .sklad_list .sklad .content').hide();
				$('.mainpage_price_storages .sklad_list .sklad .header .closeopen').text('Развернуть');
				$('.mainpage_price_storages .sklad_list .sklad.sklad_'+e.get('target').options.get('skladId')+' .content').slideDown();
				$('.mainpage_price_storages .sklad_list .sklad.sklad_'+e.get('target').options.get('skladId')+' .header .closeopen').text('Свернуть');
			});			
...
<div id="map_2" style="width: 100%; height: 400px; margin-top: -30px;"></div>
