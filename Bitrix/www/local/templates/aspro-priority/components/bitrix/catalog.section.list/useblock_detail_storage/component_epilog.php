...
if (!$arResult["SECTION"]["UF_LINK_REGION"] || in_array($arParams["UF_LINK_REGION"], $arResult["SECTION"]["UF_LINK_REGION"]) === false) {
...
if ($arResult["SECTION"]["IBLOCK_SECTION_ID"] != "69") {
    Bitrix\Iblock\Component\Tools::process404(
        'Страница не найдена'
        , true
        , true
...
<?php if(!$this->__template)  $this->InitComponentTemplate();?>
...
<div class="maxwidth-theme" style="margin-top: 30px">
    ...
    $APPLICATION->IncludeComponent("bitrix:catalog.section.list", "select_storage_map", array(
        ...
        "FILTER_NAME" => "arRegionLinkUF",    // Имя массива со значениями фильтра разделов
