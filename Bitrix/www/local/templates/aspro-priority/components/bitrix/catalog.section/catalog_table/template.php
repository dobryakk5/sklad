							...
							<tbody>
								<tr>
									...
									<td class="but-cell item_<?=$arItem["ID"]?>">
										<div class="counter_wrapp pull-right">
											<?if($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] == 'qwe' && !count($arItem["OFFERS"]) && $arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"]):?>
												<div class="counter_block" data-item="<?=$arItem["ID"];?>" <?=(in_array($arItem["ID"], $arParams["BASKET_ITEMS"]) ? "style='display: none;'" : "");?>>
													...
													<span class="plus ctrl" <?=($arAddToBasketData["MAX_QUANTITY_BUY"] ? "data-max='".$arAddToBasketData["MAX_QUANTITY_BUY"]."'" : "")?>>+</span>
												</div>
											<?endif;?>
