...
<div class="rental_catalog">
	<div class="ajaxPreloader"></div>
	<div class="ajax_rentalCatalogList">
<?}?>
		...
		<div class="rental_catalog_list">
							...
							<div class="col-md-3 col-xs-12">
								...
								<?if(strlen($arResult["SKLAD_LIST"][$arItem["IBLOCK_SECTION_ID"]]["UF_RECEPTION"]) > 0) {?>
									<div class="sklad_reception">
										<div class="lbl">Режим работы ресепшена:</div>
							...
							<div class="col-md-2 col-xs-12">						
								...
								<?if($arItem["PROPERTIES"]["STATUS"]["VALUE_ENUM_ID"] == 346) {?>									
									<div class="buy_block">
										...
										<div class="counter">
											<div class="wrap">
												...
												<div class="input"><input type="text" value="1" class="countMonths" data-price="<?=$arItem["MIN_PRICE"]["DISCOUNT_VALUE"]?>" /></div>
									...
									<div class="buy_button">
										<input type="hidden" class="box-id" value="<?=$arItem["ID"]?>" />
										<input type="hidden" class="box-name" value="<?=$arItem["NAME"]?>" />
										<input type="hidden" class="box-price" value="<?=$arItem["MIN_PRICE"]["DISCOUNT_VALUE"]?>" />
										<input type="hidden" class="box-brand" value="<?=$arResult["SKLAD_LIST"][$arItem["IBLOCK_SECTION_ID"]]["NAME"]?>" />
										<input type="hidden" class="box-category" value="<?=$arResult["SKLAD_LIST"][$arItem["IBLOCK_SECTION_ID"]]["NAME"]?>" />
										<a class="btn btn-default btn-xs add_to_cart" href="javascript:void(0);" data-product-id="<?=$arItem["ID"]?>">Арендовать</a>										
										<span style="display:none;" class="add_to_cart_popup" data-event="jqm" data-param-product-id="<?=$arItem["ID"]?>" data-param-count="1" data-param-type="box" data-name="box"></span>
									</div>
									<div class="buy_button">
										<span class="btn btn-transparent btn-xs buy_one_click">Арендовать в 1 клик</span>
										<span style="display:none;" class="buy_one_click_popup" data-event="jqm" data-param-id="51" data-param-product-id="<?=$arItem["ID"]?>" data-param-count="1" data-param-type="buy_one_click" data-name="buy_one_click"></span>
									</div>									
								<?}?>
							</div>				
						</div>
			...
			<?} else {?>
				...
				<?if($arResult["ID"] > 0) {?>
					<script>
					$(document).ready(function() {
						var $range_size = $('.box_filter .filter_block .range_<?=$arParams["PROP_SIZE"]?>');
						$('.rental_catalog_list .button .SHOW_FORM_REMINDER').attr('data-param-square-from', $range_size.data("from"));
						$('.rental_catalog_list .button .SHOW_FORM_REMINDER').attr('data-param-square-to', $range_size.data("to"));
					})
					</script>
					<div class="button">
						<span class="btn btn-default SHOW_FORM_REMINDER" data-event="jqm" data-param-webform-id="15" data-param-sklad-id="<?=$arResult["ID"]?>" data-param-square-from="" data-param-square-to="" data-param-prop-size="<?=$arParams["PROP_SIZE"]?>" data-param-type="webform" data-name="webform">Сообщить об освобождении</span>
						<span>&nbsp;&nbsp;&nbsp;ИЛИ&nbsp;&nbsp;&nbsp;измените диапазон поиска по размеру бокса</span>
					</div>
				<?}?>
			<?}?>
		</div>
...
<?}?>
<? if($arResult["UF_TEXT_BLOCK"]):?>
<section class="dop-text-block">
