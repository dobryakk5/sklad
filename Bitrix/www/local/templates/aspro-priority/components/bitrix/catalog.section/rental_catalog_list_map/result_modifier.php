...
<?
//Записываем в массив ID всех найденных боксов (это будут свободные, подходящие под фильтр)
$arFilteredBoxesId = Array();
foreach($arResult["ITEMS"] as $key=>$arItem) {
	$arResult["ITEMS"][$key]["CLASS_STATUS"] = "opened filtered";
	$arFilteredBoxesId[] = $arItem["ID"];
}

//Получаем все остальные боксы этого склада и этого этажа
$arOtherItems = Array();
if((strlen($arParams["SECTION_CODE"]) > 0) and (strlen($arParams["FLOOR_CODE"]) > 0)) {
	$res = CIBlockElement::GetList(Array("propertysort_STATUS"=>"desc"), Array("IBLOCK_ID"=>STORAGES_CATALOG_IBLOCK, "SECTION_CODE"=>$arParams["SECTION_CODE"], "PROPERTY_FLOOR_VALUE"=>preg_replace("/[^0-9]/", "", $arParams["FLOOR_CODE"])." этаж", "!ID"=>$arFilteredBoxesId), false, Array(), Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_STATUS", "PROPERTY_MAP_COORDS"));
...
foreach($arOtherItems as $item) {
	if($item["PROPERTY_STATUS_ENUM_ID"] != BOX_STATUS_OPENED_ID) {
		$classStatus = "closed";
	} else {
		$classStatus = "opened";	
	}
