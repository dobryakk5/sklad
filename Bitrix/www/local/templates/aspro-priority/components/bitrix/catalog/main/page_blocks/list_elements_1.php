		...
		<?if($arSeoItem["PROPERTY_TIZERS_VALUE"]):?>
			...
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					"IBLOCK_ID" => CNextCache::$arIBlocks[SITE_ID]["aspro_priority_content"]["aspro_priority_tizers"][0],
					"NEWS_COUNT" => "4",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "ASC",
					"SORT_BY2" => "ID",
...
if($isAjaxFilter == "Y")
	$isAjax="N";?>
<?$section_pos_top = \Bitrix\Main\Config\Option::get("aspro.priority", "TOP_SECTION_DESCRIPTION_POSITION", "UF_SECTION_DESCR", SITE_ID );?>
<?$section_pos_bottom = \Bitrix\Main\Config\Option::get("aspro.priority", "BOTTOM_SECTION_DESCRIPTION_POSITION", "DESCRIPTION", SITE_ID );?>
<?if($itemsCnt):?>
	<?
	//sort
	ob_start();
	...
	<?if('VERTICAL' === "VERTICAL"):?>
		...
		<?$html = ob_get_clean();?>
<?
        $this->__component->__template->SetViewTarget(isMobile()?'mobile_filter':'under_sidebar_content');
        echo $html;
	...
	<div class="right_block1 clearfix catalog <?=strtolower($arTheme["FILTER_VIEW"]["VALUE"]);?>" id="right_block_ajax">
		<?if('HORIZONTAL2' === "HORIZONTAL" || $arTheme["FILTER_VIEW"]["VALUE"] === "COMPACT"){?>
			<div class="<?=("HORIZONTAL2"=="HORIZONTAL" ? 'filter_horizontal' : '');?><?=($arTheme["FILTER_VIEW"]["VALUE"]=="COMPACT" ? ' filter_compact' : '');?> swipeignore">
				<?include_once(__DIR__."/../filter.php")?>
			</div>
		<?}/*else{?>
...
<?if($itemsCnt):?>
			...
			<?}?>
            <?if(isMobile()):?>
                <?$APPLICATION->ShowViewContent('mobile_filter');?>
            <?endif;?>
			<?// sort?>
			<?=$sortHtml;?>
