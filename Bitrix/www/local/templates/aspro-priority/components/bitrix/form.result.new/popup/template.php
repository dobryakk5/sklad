...
<div class="form popup <?=$arResult["arForm"]["SID"]?> bxform<?=($arResult['FORM_NOTE'] ? ' success' : '')?><?=($arResult['isFormErrors'] == 'Y' ? ' error' : '')?>">
	<div class="wrap">
			...
			<div class="form-header">
				<div class="text <?=($arResult["isFormErrors"] == "Y" ? 'error' : 'success')?>">
					...
					<?else:?>
						...
						//цели custom
						$targetName = "";
						if(in_array($arResult["arForm"]["ID"], Array(11,12,13,14))) {
							$targetName = "form_formManagerOrder";
						}
						if(in_array($arResult["arForm"]["ID"], Array(15))) {
							$targetName = "form_formManagerOrder_5";
						}
						...
						$eventName = array(
							'11' => 'form_formManagerOrder', // Аренда бокса через менеджера
							'12' => 'form_formManagerOrder', // Оставьте свои контактные данные, и с вами свяжется менеджер для консультации по аренде бокса
							'13' => 'form_formManagerOrder', // Отправить информацию менеджеру
							'14' => 'form_formManagerOrder', // Отправьте заявку прямо сейчас
							'15' => 'form_formManagerOrder_5', // Сообщить об освобождении бокса
							'23' => 'delivery_request', // Заказать доставку
						);
						if (isset($eventName[$arResult["arForm"]["ID"]])):
						  $json = json_encode(array(
								'event' => $eventName[$arResult["arForm"]["ID"]],
								'formNumber' => $arResult["arForm"]["ID"]
							));
							?>
				...
				<?if( $arParams["DISPLAY_CLOSE_BUTTON"] == "Y" ){?>
					<div class="button">
						<button class="btn-lg btn btn-default refresh-page jqmClose"><?=($arParams["CLOSE_BUTTON_NAME"] ? $arParams["CLOSE_BUTTON_NAME"] : GetMessage("RELOAD_PAGE"));?></button>
					</div>
				<?}?>
			</div>
		<?}?>
		<?if(!$arResult["FORM_NOTE"]){?>
			<div class="form-header" data-id="<?=$arResult["arForm"]["ID"];?>">
				<?if($arResult["isFormTitle"] == "Y"):?>
					<div class="text">
						<div class="title"><?=$arResult["FORM_TITLE"]?></div>
					</div>
			...
			<div class="form-body">
				<?if(is_array($arResult["QUESTIONS"])):?>				
					...
					<?if($arResult["arForm"]["ID"] == "23") {?>
						<style>
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_1"],
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_2"],
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_1"],
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_2"],
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="CARGO"],
							form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="PACKING"] {
								display: none;
							}						
						</style>
						<script type="text/javascript">
							$(document).ready(function() {
								$('html').on('change', 'form[name="<?=$arResult["arForm"]["VARNAME"]?>"] input[name="form_radio_ACTION_TYPE"]', function() {
									var typeID = $(this).attr('id');
									if(typeID == "137") {
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_1"]').show().find('input, textarea, select').prop('disabled', false);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_1"]').show().find('input, textarea, select').prop('disabled', false);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_2"]').hide().find('input, textarea, select').prop('disabled', true);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_2"]').hide().find('input, textarea, select').prop('disabled', true);										
									}
									if(typeID == "138") {
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_1"]').hide().find('input, textarea, select').prop('disabled', true);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_1"]').hide().find('input, textarea, select').prop('disabled', true);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_FROM_2"]').show().find('input, textarea, select').prop('disabled', false);
										$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="LOCATION_TO_2"]').show().find('input, textarea, select').prop('disabled', false);										
									}									
									$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="CARGO"]').show();
									$('form[name="<?=$arResult["arForm"]["VARNAME"]?>"] .form-body .row[data-sid="PACKING"]').show();
								})
							});
						</script>
						<? 
						...
						<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {?>
							<?if($FIELD_SID == "ACTION_TYPE") {?>
								<div class="row" data-sid="<?=$FIELD_SID?>">
									<div class="col-md-12">
										<div class="form-group option">											
											<label><span><?=$arQuestion["CAPTION"]?>&nbsp;<span class="required-star">*</span></span></label>											
											<?foreach($arQuestion["STRUCTURE"] as $radio) {?>
												<div>
													<input type="radio" id="<?=$radio["ID"]?>" name="form_radio_<?=$FIELD_SID?>" value="<?=$radio["ID"]?>" required >
													<label for="<?=$radio["ID"]?>"><?=$radio["MESSAGE"]?></label>
												</div>
											<?}?>										
										</div>
									</div>
								</div>
                    <?} elseif ($FIELD_SID == "PACKING") {?>
                        <div class="row" data-sid="<?=$FIELD_SID?>">
                            <div class="col-md-12">
                                <div class="form-group option">
                                    <label><span><?=$arQuestion["CAPTION"]?></label>
                                    <?foreach($arQuestion["STRUCTURE"] as $checkbox) {?>
                                        <div class="bx_filter">
                                            <input type="checkbox" data-sid="<?=$FIELD_SID?>" id="<?=$checkbox["ID"]?>" name="form_checkbox_<?=$FIELD_SID?>[]" value="<?=$checkbox["ID"]?>" >
                                            <label for="<?=$checkbox["ID"]?>"><?=$checkbox["MESSAGE"]?></label>
                                        </div>
                                    <?}?>
                                </div>
                            </div>
                        </div>
                    <?} elseif ($FIELD_SID == "DELIVERY") {?>
                        <div class="row delivery" data-sid="<?=$FIELD_SID?>">
                            <div class="col-md-12">
                                <div class="form-group option">
                                    <label><span><?=$arQuestion["CAPTION"]?></label>
                                    <?foreach($arQuestion["STRUCTURE"] as $checkbox) {?>
