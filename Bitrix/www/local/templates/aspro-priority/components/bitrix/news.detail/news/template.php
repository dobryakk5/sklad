...
<? if (true): ?>
    <? ob_start(); ?>
    <div class="ask_a_question border shadow calc">
        <div class="inner">
            <div class="text-block">
                <div class="introtext">Подобрать бокс</div>
                <p>
                    Выберите сдаваемые на хранение вещи для уточнения необходимого размера бокса
                </p>
            </div>
        </div>
        <div class="outer">
            <span class="font_upper"><a href="/price/">Калькулятор по параметрам</a></span>
        </div>
    </div>
    <? $sFormQuestion = ob_get_contents();
    ob_end_clean(); ?>
<? elseif (\Bitrix\Main\Loader::includeModule('subscribe')): ?>
    <? ob_start(); ?>
