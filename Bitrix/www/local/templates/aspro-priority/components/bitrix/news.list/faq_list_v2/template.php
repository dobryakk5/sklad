        ...
        <div class="tabs">
            <ul class="nav nav-tabs font_upper_md">
                ...
                <? foreach ($arResult['SECTIONS'] as $SID => $arSection): ?>
                    <? if (!$SID) continue; ?>
                    <li class="shadow border  <?= $i++ == 0 ? 'active' : '' ?>"><a
                                onclick="tabshow(event,$(this));return false;"
                                href="#<?= (strlen($arSection["CODE"]) > 0) ? $arSection["CODE"] : $this->GetEditAreaId($arSection['ID']) ?>"><?= $arSection['NAME'] ?></a>
                    </li>
                <? endforeach; ?>
            </ul>
            <? endif; ?>
            ...
            <div class="<?= ($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content') ?>">
                ...
                <? foreach ($arResult['SECTIONS'] as $SID => $arSection): ?>
                    ...
                    ?>
                    <div id="<?= (strlen($arSection["CODE"]) > 0) ? $arSection["CODE"] : $this->GetEditAreaId($arSection['ID']) ?>"
                        ...
                        <div class="title-tab-heading border shadow visible-xs"><?= $arSection['NAME']; ?><span
                                ...
                                <? foreach ($arSection['ITEMS'] as $i => $arItem): ?>
                                    ...
                                    <? if ($arParams['FORM'] == 'Y'): ?>
                                        <button class="btn btn-default" data-event="jqm" data-name="resume"
                                                ...
                                                data-autoload-POST="<?= $arItem['NAME'] ?>"
                                    ...
                                    <? if ($arParams['VIEW_TYPE'] == 'list'): ?>
                                        <div class="col-md-12">
                                            <div class="item border shadow<?= ($bImage ? '' : ' wti') ?> clearfix noborder"
                                                ...
                                                <div class="body-info">
                                                    ...
                                                    <? if ($bDetailLink): ?>
                                                        <div class="link-block-more">
                                                            <a href="<?= $arItem['DETAIL_PAGE_URL'] ?>"
                                                               class="btn-inline sm rounded black"><?= GetMessage('TO_ALL') ?>
                                    ...
                                    <? elseif ($arParams['VIEW_TYPE'] == 'table'): ?>
                                        <div class="col-md-<?= floor(12 / $arParams['COUNT_IN_LINE']) ?> col-sm-<?= floor(12 / round($arParams['COUNT_IN_LINE'] / 2)) ?>">
                                            <div class="item border shadow<?= ($bImage ? '' : ' wti') ?>"
                                                 id="<?= $this->GetEditAreaId($arItem['ID']) ?>">
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <? if (!$bImage): ?>
                                        ...
                                        <div class="accordion-type-1" itemprop="mainEntity" itemscope itemtype="https://schema.org/Question">
                                            <div class="item border shadow<?= ($bImage ? '' : ' wti') ?>"
                                                ...
                                                <div class="accordion-head accordion-close" data-toggle="collapse"
                                                     data-parent="#accordion<?= $arSection['ID'] ?>"
                                                     href="#accordion<?= $arItem['ID'] ?>_<?= $arSection['ID'] ?>">
                                                    ...
                                                    <? if (in_array('PAY', $arParams['PROPERTY_CODE'])): ?>
                                                        <div class="pay">
                                                            <? if ($arItem['DISPLAY_PROPERTIES']['PAY']['VALUE']): ?>
                                                                <?= GetMessage('PAY_ABOUT') ?>&nbsp;
                                                                <b><?= $arItem['DISPLAY_PROPERTIES']['PAY']['VALUE'] ?></b>
                                                            <? else: ?>
                                                ...
                                                <div id="accordion<?= $arItem['ID'] ?>_<?= $arSection['ID'] ?>"
                                                    ...
                                                    <div class="accordion-body">
                                                        <div class="row"  itemprop="acceptedAnswer" itemscope="" itemtype="http://schema.org/Answer">
                                                            ...
                                                            <? elseif ($arParams["IMAGE_POSITION"] == "right"): ?>
                                                                ...
                                                                <div class="col-md-3" itemprop="text"><?= $imagePart ?></div>
                                                            <? else: ?>
