...
<?if($arResult['ITEMS']):?>
	...
	<div class="banners-big front<?=($bHideOnNarrow ? ' hidden_narrow' : '')?>">
		<div class="maxwidth-banner">
			<div class="flexslider unstyled <?=($animationDirection == 'vertical' ? 'vertical' : '')?>" data-plugin-options='{"video": true, "directionNav": true, "customDirection": ".nav-carousel a", "controlNav": true, "touch": <?= count($arResult['ITEMS']) > 1 ? 'true' : 'false' ?>, <?=($bAnimation ? '"slideshow": true,' : '"slideshow": false,')?> <?=($animationType ? '"animation": "'.$animationType.'",' : '')?> <?=($animationDirection ? '"direction": "'.$animationDirection.'",' : '')?> <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "animationLoop": true}'>
											...
											<?elseif($bShowButton1 || $bShowButton2):?>
												...
												<?if($bShowButton1):?>
													<a href="<?=$arItem['PROPERTIES']['BUTTON1LINK']['VALUE']?>" 
														class="btn 
															<?=(strlen($arItem['PROPERTIES']['BUTTON1CLASS']['VALUE_XML_ID']) ? $arItem['PROPERTIES']['BUTTON1CLASS']['VALUE_XML_ID'] : 'btn-default')?>
															<?=(strlen($arItem['PROPERTIES']['BUTTON1CLASS2']['VALUE']) ? $arItem['PROPERTIES']['BUTTON1CLASS2']['VALUE'] : '')?>"
														<?php if($arItem['PROPERTIES']['BUTTON1JAVA']['VALUE']): ?>
															onclick="<?= $arItem['PROPERTIES']['BUTTON1JAVA']['VALUE']; ?>"
														<?php endif; ?>
														<?php if($arItem['PROPERTIES']['BUTTON1ATTRIBUTES']['VALUE']): ?>
															<?= $arItem['PROPERTIES']['BUTTON1ATTRIBUTES']['VALUE']; ?>
														<?php endif; ?>
													>
												...
												<?if($bShowButton2):?>
													<a href="<?=$arItem['PROPERTIES']['BUTTON2LINK']['VALUE']?>" 
														class="btn 
															<?=(strlen($arItem['PROPERTIES']['BUTTON2CLASS']['VALUE_XML_ID'] ) ? $arItem['PROPERTIES']['BUTTON2CLASS']['VALUE_XML_ID'] : 'btn-default white')?>
															<?=(strlen($arItem['PROPERTIES']['BUTTON2CLASS2']['VALUE']) ? $arItem['PROPERTIES']['BUTTON2CLASS2']['VALUE'] : '')?>"
													>
									...
									<?if(is_array($arItem['PREVIEW_PICTURE'])):?>
										<?if($bLinkOnName):?>
											<a href="<?=$arItem['PROPERTIES']['LINKIMG']['VALUE']?>" class="image">
												<img class="plaxy lazy" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" alt="<?=($arItem['PREVIEW_PICTURE']['DESCRIPTION'] ? $arItem['PREVIEW_PICTURE']['DESCRIPTION'] : $arItem['NAME'])?>" />
											</a>
										<?else:?>
											<img class="plaxy lazy" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" alt="<?=($arItem['PREVIEW_PICTURE']['DESCRIPTION'] ? $arItem['PREVIEW_PICTURE']['DESCRIPTION'] : $arItem['NAME'])?>" title="<?=($arItem['PREVIEW_PICTURE']['TITLE'] ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" />
										<?endif;?>
									<?endif;?>
									<?$img = ob_get_clean();?>
