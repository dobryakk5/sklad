	...
	<div class="mix_banners bottom">
		<? foreach ($arResult['ITEMS'] as $arItem): ?>
			...
			<? if (strlen($arItem['PREVIEW_PICTURE']['SRC'])): ?>
				<div class="lazy item<?= ($textColor ? ' ' . $textColor : '') ?><?= ($bWideBanner ? ' wide' : '') ?>" id="<?= $this->GetEditAreaId($arItem['ID']) ?>" data-bg="url(<?= $arItem['PREVIEW_PICTURE']['SRC'] ?>)" style="background-size:cover; background-repeat:no-repeat; background-position-x:top; background-position-y:center;">
					<div class="text">
