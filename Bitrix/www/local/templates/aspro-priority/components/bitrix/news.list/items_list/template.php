...
<div class="item-views <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>" <?=($arResult['ID'] == 9 ? 'itemscope itemtype="https://schema.org/FAQPage"' : '')?>>
	<?// top pagination?>
			...
			<div class="tabs">
				<ul class="nav nav-tabs">
					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						<?if(!$SID) continue;?>
						<li class="shadow border<?=$i++ == 0 ? ' active' : ''?>"><a data-toggle="tab" href="#<?=$this->GetEditAreaId($arSection['ID'])?>"><?=$arSection['NAME']?></a></li>
					<?endforeach;?>
				</ul>
		<?endif;?>
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?endif;?>
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									// preview picture
									$bImage = (isset($arItem['FIELDS']['PREVIEW_PICTURE']) && $arItem['PREVIEW_PICTURE']['SRC'] ? true : false);
									$imageSrc = ($bImage ? $arItem['PREVIEW_PICTURE']['SRC'] : SITE_TEMPLATE_PATH.'/images/noimage.png');
									$imageDetailSrc = ($bImage && !$imageSrc && $arItem['DETAIL_PICTURE'] ? $arItem['DETAIL_PICTURE']['SRC'] : false);
									...
									<?ob_start();?>
										<?// element name?>
										<?if($arParams['VIEW_TYPE'] != 'accordion' && strlen($arItem['FIELDS']['NAME'])):?>
											<div class="title">
										...
										<div class="previewtext">
											<?if(strlen($arItem['FIELDS']['PREVIEW_TEXT']) || strlen($arItem['FIELDS']['DETAIL_TEXT'])):?>
												<div <?=($arResult['ID'] == 9 ? 'itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer"' : '')?>>
													<?if(strlen($arItem['FIELDS']['PREVIEW_TEXT'])):?>
														<?if($arItem['PREVIEW_TEXT_TYPE'] == 'text'):?>
															<p  <?=($arResult['ID'] == 9 ? 'itemprop="text"' : '')?>><?=$arItem['FIELDS']['PREVIEW_TEXT']?></p>
														<?else:?>
															<div <?=($arResult['ID'] == 9 ? 'itemprop="text"' : '')?>><?=$arItem['FIELDS']['PREVIEW_TEXT']?></div>
														<?endif;?>
													<?endif;?>
												</div>
										...
										<?if($arItem['DISPLAY_PROPERTIES']):?>
											<div class="properties">
												<?foreach($arItem['DISPLAY_PROPERTIES'] as $PCODE => $arProperty):?>
													<?if(in_array($PCODE, array('PERIOD', 'TITLE_BUTTON', 'LINK_BUTTON', 'RECORD_BUTTON'))) continue;?>
									...
									<?if($arParams['VIEW_TYPE'] == 'list'):?>
										<div class="col-md-12">
											<div class="item shadow1<?=($bImage ? '' : ' wti')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
												<?if($bImage):?>
													<?=$imagePart?>
												<?endif;?>
												<div class="body-info">
									...
									<?elseif($arParams['VIEW_TYPE'] == 'table'):?>
										<div class=" col-md-<?=floor(12 / $arParams['COUNT_IN_LINE'])?> col-sm-<?=floor(12 / round($arParams['COUNT_IN_LINE'] / 2))?>">
											<div class="item shadow1<?=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
												<?if(!$bImage):?>
									...
									<?elseif($arParams['VIEW_TYPE'] == 'accordion'):?>
										<div class="accordion-type-1">
											<div class="item  border shadow<?=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>" <?=($arResult['ID'] == 9 ? 'itemscope itemprop="mainEntity" itemtype="https://schema.org/Question"' : '')?>>
												<div class="accordion-head accordion-close clearfix" data-trigger="opener" data-toggle="collapse" data-parent="#accordion<?=$arSection['ID']?>" href="#accordion<?=$arItem['ID']?>_<?=$arSection['ID']?>">
													<span class="arrow_open pull-right"></span>
													<div class="title" <?=($arResult['ID'] == 9 ? 'itemprop="name"' : '')?>><span><?=$arItem['NAME']?></span></div>
												...
												<div id="accordion<?=$arItem['ID']?>_<?=$arSection['ID']?>" class="panel-collapse collapse">
													<div class="accordion-body">
														...
														<?if(isset($arItem['DISPLAY_PROPERTIES']['RECORD_BUTTON']) && $arItem['DISPLAY_PROPERTIES']['RECORD_BUTTON']['VALUE_XML_ID'] == 'YES'):?>
															<div class="row">
																<div class="col-md-12">
																	<div class="button">
																		<span class="btn btn-default btn animate-load" data-event="jqm" data-param-id="<?=($arParams["FORM_ID_RECORD_ONLINE"] ? $arParams["FORM_ID_RECORD_ONLINE"] : CCache::$arIBlocks[SITE_ID]['aspro_medc2_form']['aspro_medc2_online'][0]);?>" data-name="record_online"><span><?=(strlen($arParams['S_RECORD_ONLINE']) ? $arParams['S_RECORD_ONLINE'] : Loc::getMessage('S_RECORD_ONLINE'))?></span></span>
																	</div>
																</div>
															</div>
														<?endif?>
