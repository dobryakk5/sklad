        ...
        <table class="table">
            ...
            foreach ($arResult["ITEMS"] as $arItem) { ?>
                <tr>
                    ...
                    <td>
                        <?
                        if ($arItem["PROPERTIES"]["STATUS"]['VALUE_ENUM_ID'] != 353) {
                            if (is_countable($arItem["INVOICES"]) && count($arItem["INVOICES"]) > 0) {
//                                foreach ($arItem["INVOICES"] as $arInvoice) {
//                                    echo '<div class="invoice_link">Счет за <a href="/cabinet/docs/invoices_not_paid/">' . $arInvoice["MONTH"] . ' г.</a></div>';
//                                }
                                echo '<div class="invoice_link"><a href="/cabinet/docs/invoices_not_paid/">Счета к оплате</a></div>';
                            } else {
                                echo '-';
                            }
                        } else {
