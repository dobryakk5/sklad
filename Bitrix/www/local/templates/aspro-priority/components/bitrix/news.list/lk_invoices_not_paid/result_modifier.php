...
foreach($arResult["ITEMS"] as $key=>$arItem) {
	...
	if($arInvoicePrice = $rsPrice->fetch()) {
		$arResult["ITEMS"][$key]["PRICE"] = $arInvoicePrice["PRICE"];
		$arResult["ITEMS"][$key]["PRICE_FORMATTED"] = FormatCurrency($arInvoicePrice["PRICE"], $arInvoicePrice["CURRENCY"]);
		$arResult["OVERALL_SUM"] = $arResult["OVERALL_SUM"] + $arInvoicePrice["PRICE"];
	}
}

if($arResult["OVERALL_SUM"] > 0) {
	$arResult["OVERALL_SUM_FORMATTED"] = FormatCurrency($arResult["OVERALL_SUM"], "RUB");
}
?>