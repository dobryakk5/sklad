...
<?if($arResult['ITEMS']):?>
	<div class="item-views sales linked sections news_list_1" itemscope itemtype="https://schema.org/CollectionPage">
		
		<meta itemprop="name" content="Новости компании АльфаСклад">
		<meta itemprop="description" content="Актуальные новости и события компании">
		...
		<div class="items row">
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				?>
				<div class="col-md-12" itemscope itemtype="https://schema.org/NewsArticle">
						...
						<div class="item<?=($imageSrc ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
							<?if($imageSrc):?>
								<div class="image" itemscope itemtype="https://schema.org/ImageObject">
									<?if($bDetailLink):?>
										<a href="<?=$arItem['DETAIL_PAGE_URL']?>" itemprop="contentUrl">
									<?endif;?>
										<img src="<?=$imageSrc?>" 
							...
							<div class="info">
								...
								<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && strlen($arItem['FIELDS']['PREVIEW_TEXT'])):?>
									<div class="previewtext" itemprop="articleBody">
								...
								<?if($bActiveDate):?>
									<div class="period font_xs">
										<?if(strlen($arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE'])):?>
											<span class="date" itemprop="datePublished"><?=$arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE']?></span>
										<?else:?>
											<span class="date" itemprop="datePublished"><?=$arItem['DISPLAY_ACTIVE_FROM']?></span>
										<?endif;?>
									</div>
								<?endif;?>								
							</div>
