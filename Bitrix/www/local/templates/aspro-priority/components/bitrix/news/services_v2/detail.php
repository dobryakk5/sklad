    ...
    <div class="detail <?= ($templateName = $component->{'__template'}->{'__name'}) ?>">
        <? if ($arParams["USE_SHARE"] == "Y" && $arElement): ?>
            ...
            <script type="text/javascript">
                $('#pagetitle').addClass('shares');
                $(document).ready(function() {
                    if ($('a.rss').length)
                        $('a.rss').after($('.share.top'));
                    else
                        $('#pagetitle').before($('.share.top'));
                    $('.share.top').show();
                })
            </script>
        <? endif; ?>
        ...
        <div class="row">
            ...
            <div class="col-md-6 col-sm-6 col-xs-6">
                <a class="back-url url-block font_upper"
                    href="<?= $list_url; ?>"><span><?= GetMessage('BACK_LINK') ?></span></a>
            </div>
        </div>
    </div>
</div>
...
if (strlen($arElement["ID"]) > 0) {
    // добавляем свою кнопку на верхний баннер
    //кэшируем
    $obCache = new CPHPCache();
    $cacheLifetime = 7200;
    $cacheID = "service_data_" . $arElement["ID"];
    $cachePath = "/service_data/";
    if ($obCache->InitCache($cacheLifetime, $cacheID, $cachePath)) {
        $vars = $obCache->GetVars();
        $arAdditionalButton = $vars["ADDITIONAL_BUTTON"];
    } elseif ($obCache->StartDataCache()) {
        global $CACHE_MANAGER;
        $CACHE_MANAGER->StartTagCache($cachePath);

        $db_prop = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $arElement["ID"], array("sort" => "asc"), array("CODE" => "ADDITIONAL_BUTTON"));
        if ($arAdditionalButton = $db_prop->Fetch()) {
            $obCache->EndDataCache(array("ADDITIONAL_BUTTON" => $arAdditionalButton));
        }
    ...
    if (strlen($arAdditionalButton["DESCRIPTION"]) > 0) {
?>
        <script>
            $(document).ready(function() {
                $('.maxwidth-banner .text.item_block .buttons').append('<span><a class="btn btn-default white btn-lg" style="margin-bottom:20px;" href="<?= $arAdditionalButton["VALUE"] ?>"><span><?= $arAdditionalButton["DESCRIPTION"] ?></span></a></span>');
            });
        </script>
    <?
    }
    ...
    //подменяем кнопки "Заказать услугу" для элемента с ID = 146 (Доставка) на кнопку "Заказать доставку"
    if ($arElement["ID"] == "146") {
    ?>
        <script>
            $(document).ready(function() {
                var btn = $('.btn[data-name="order_services"]');
                btn.parent().empty().append('<span class="btn btn-default animate-load" title="Заказать доставку" data-event="jqm" data-param-webform-id="23" data-param-type="webform" data-name="webform">Заказать доставку</span>');
            });
        </script>
<?
    }
