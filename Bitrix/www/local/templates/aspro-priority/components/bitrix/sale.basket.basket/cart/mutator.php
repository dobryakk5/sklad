...
foreach ($this->basketItems as $row)
{
		...
		foreach ($result['GRID']['HEADERS'] as &$value)
		{
			...
			elseif (!empty($row[$value['id']]))
			{
				...
				$rowData['COLUMN_LIST'][] = array(
					...
					'IS_TEXT' => !$isHtml,
					'IS_HTML' => $isHtml,
					'HIDE_MOBILE' => !isset($mobileColumns[str_replace("_VALUE","",$value['id'])])
				);
			}
		}
	}
	...
	/********* CUSTOM **********/
	$resProd = CIBlockElement::GetByID($row["PRODUCT_ID"]);
	if($arProd = $resProd->GetNext()) {
		if(strlen($arProd["IBLOCK_SECTION_ID"]) > 0) {
			if($arProd["IBLOCK_ID"] == STORAGES_CATALOG_IBLOCK) {
				//получение данных о складе
				$rsSect = CIBlockSection::GetList(array("sort"=>"asc"), Array("IBLOCK_ID"=>$arProd["IBLOCK_ID"], "DEPTH_LEVEL"=>1, "ID"=>$arProd["IBLOCK_SECTION_ID"]), false, Array("ID", "IBLOCK_ID", "NAME", "CODE", "UF_ADDRESS"));
				if($arSect = $rsSect->GetNext()) {
				   $rowData["SKLAD_NAME"] = $arSect["NAME"];
				   $rowData["SKLAD_ADDRESS"] = $arSect["UF_ADDRESS"];
				}
				//склонение месяцев
				$nameMeasure = trim(preg_replace("/[0-9]+/", "", num_decline($rowData["QUANTITY"], Array("месяц","месяца","месяцев")))); 
				$rowData["MEASURE_TEXT_DECLINE"] = $nameMeasure;			
			}
		}			
	}
	/**********************************/
