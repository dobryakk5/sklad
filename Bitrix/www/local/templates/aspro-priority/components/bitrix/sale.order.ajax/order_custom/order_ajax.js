	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				//ТУТ this.getProfilesControl(regionNodeCol);
		...
		/**
		 * функция для ввода только цифровых значений в поля типа текст
		 * @param e
		 */
		checkPassportFieldValue: function(e)
		{
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				if ( (inputs[i].id == 'soa-property-9') || inputs[i].id == 'soa-property-10') {
					BX.bind(inputs[i], 'keyup', BX.delegate(this.checkPassportFieldValue, this));
				}
			}
