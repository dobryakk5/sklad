 ...
 */
$this->addExternalJS("/bitrix/templates/aspro-priority/js/jquery.maskedinput.min.js");
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
            <div class="col-sm-9 ">
                <div class="alert dang">
                    <span class="dang-sp"></span>
                    <div>
                    Если в данный момент <b>вы находитесь за границей</b>, возможны сложности с оплатой счета через личный кабинет.
                    В этом случае мы рекомендуем использовать вам как основной способ - оплату при помощи
                    сканирования QR-кода через мобильное приложение вашего банка.
                    </div>
                </div>
			...
			<div class="col-sm-9 bx-soa">
					...
					<div class="checkbox">
						...
						if ($arParams['USER_CONSENT'] === 'Y')
						{
							...
							<label class="main-user-consent-request">
									...
									<span class="main-user-consent-request-announce">Согласен с условиями предоставления <a target="_blank" href="/upload/iblock/faa/Standartnye-usloviya-AlfaSklad.pdf">складских услуг</a> Альфасклад.</span>
							</label>
				...
				<div style="display: none;">
					...
					<input type="hidden" name="ORDER_PROP_17" value="Y">
...
<script type="text/javascript">
	$(document).ready(function(){
		if ($("input[name='ORDER_PROP_11']"))
			$("input[name='ORDER_PROP_11']").mask("9999999999");
		if ($("input[name='ORDER_PROP_12']"))
			$("input[name='ORDER_PROP_12']").mask("999999999");
		if ($("input[name='ORDER_PROP_4']"))
			$("input[name='ORDER_PROP_4']").mask("99.99.9999", {placeholder: "дд.мм.гггг" });
		if ($("input[name='ORDER_PROP_7']"))
			$("input[name='ORDER_PROP_7']").mask("+9(999)999-99-99");
		if ($("input[name='ORDER_PROP_15']"))
			$("input[name='ORDER_PROP_15']").mask("+9(999)999-99-99");

		$("body" ).on( "focus", "input[name='ORDER_PROP_11']", function() {
			 $(this).mask("9999999999");
		});
		$("body" ).on( "focus", "input[name='ORDER_PROP_12']", function() {
			 $(this).mask("999999999");
		});
		$("body" ).on( "focus", "input[name='ORDER_PROP_4']", function() {
			 $(this).mask("99.99.9999", {placeholder: "дд.мм.гггг" });
		});

		$("body" ).on( "focus", "input[name='ORDER_PROP_7']", function() {
			 $("input[name='ORDER_PROP_7']").mask("+9(999)999-99-99");
		});
		$("body" ).on( "focus", "input[name='ORDER_PROP_15']", function() {
			 $("input[name='ORDER_PROP_15']").mask("+9(999)999-99-99");
		});
	})
</script>
...
<dialog>
  <p>Уважаемый пользователь, просим вас авторизоваться для выполнения заказа</p>
  <a class="dialog__button btn btn-default" href="/auth/?backurl=/order/" id="auth">Авторизоваться</a>
  <a href="javascript:void(0)" id="close" class="dialog__button btn btn-default">Закрыть</a>
</dialog>
<script type="text/javascript">
	$(document).ready(function(){
		...
		document.querySelector('#close').onclick = function() {
			$("input[name='ORDER_PROP_7'],input[name='ORDER_PROP_15']").val('');
		  	dialog.close();
		};

		//phone
		$("body" ).on( "focusout", "input[name='ORDER_PROP_7'],input[name='ORDER_PROP_15']", function() {
		 	var phone = $(this).val();
            var control = $(this).val().replace(/[^a-zA-Z0-9]/g, '');
		 	if(control.length==11){
				$.ajax({
		            ...
		            success: function(val){
                        console.log(val);
		                if(val==1){
		                	$('#bx-soa-orderSave .pull-right').attr('disabled', true);
		                	$('.btn-order-save').attr('disabled', true);
		                	$("input[name='ORDER_PROP_7'],input[name='ORDER_PROP_15']").val('');
		                	dialog.showModal();

		                }else{
		                	$('#bx-soa-orderSave .pull-right').attr('disabled', false);
		                	$('.btn-order-save').attr('disabled', false);
		                }
		...
		//email
		$("body" ).on( "focusout", "input[name='ORDER_PROP_8'],input[name='ORDER_PROP_14']", function() {
		 	var email = $(this).val();
		 	
			$.ajax({
				type: "POST",
				url: "/ajax/searchUserByEmail.php",
				data: {email:email},
				success: function(val){
					if(val==1){
						$('#bx-soa-orderSave .pull-right').attr('disabled', true);
						$('.btn-order-save').attr('disabled', true);
						$("input[name='ORDER_PROP_8'],input[name='ORDER_PROP_14']").val('');
						dialog.showModal();
