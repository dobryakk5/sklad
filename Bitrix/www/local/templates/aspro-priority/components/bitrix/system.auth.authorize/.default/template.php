...
<?if( $arResult["FORM_TYPE"] == "login" ){?>
    ...
    <p>Для входа в личный кабинет и оплаты счетов введите свой логин (email) и пароль, который был отправлен Вам по email при заключении договора, либо войдите через SMS (если телефон привязан к аккаунту)</p>
    <div class="tabs">
        <ul class="nav nav-tabs">
                <li class="font_upper_md shadow border active"><a href="#tab_auth_email" data-toggle="tab">Электронная почта</a></li>
                <li class="font_upper_md shadow border"><a href="#tab_auth_sms" data-toggle="tab">Через SMS</a></li>
        </ul>
        <div class="tab-content">
                <div class="tab-pane active" id="tab_auth_email">
                            ...
                            <div class="wrap_md">
                                <div class="main_info form">
                                    <div class="form-wr form-body">
                                        ...
                                        <?}?>
                                        <form id="avtorization-form" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
                                            <?if($arResult["BACKURL"] <> ''):?>
                                            ...
                                            <!--noindex--><a href="/registration/" rel="nofollow" class="btn btn-default btn-transparent btn-lg register pull-right" tabindex="6"><?=GetMessage("AUTH_REGISTER_NEW")?></a><!--/noindex-->
                                        <?endif;*/?>
                                                </div>
                                ...
                                $(document).ready(function()
                                {
                                    ...
                                    */
                                    ...
                                    try{
                                        setTimeout(function() {
                                            //LOGIN
                                            if($('#avtorization-form input[name=USER_LOGIN]').length > 0) {
                                                $('#avtorization-form input[name=USER_LOGIN]').parents('.form-group').addClass('input-filed');
                                            }
                                            //PASSWORD
                                            if($('#avtorization-form input[name=USER_PASSWORD]').length > 0) {
                                                $('#avtorization-form input[name=USER_PASSWORD]').parents('.form-group').addClass('input-filed');
                                            }
                                        }, 300);
                                    }
                                    catch(e){
        ...
        <div class="tab-pane" id="tab_auth_sms">
            <? $APPLICATION->IncludeComponent('webpult:sms.auth.form', '', ['AJAX_MODE'=>'Y']) ?>
        </div>
    </div>
<?}else{?>
	<script>
