...
<head>
    ...
    <?php /* SEO noindex for technical/filter URLs */ ?>
<?php
...
$hasSeoNoindexParams =
    count(array_intersect(array_keys($query), $seoNoindexParams)) > 0;

foreach ($query as $key => $value) {
    if (strpos($key, 'PAGEN_') === 0) {
        $hasSeoNoindexParams = true;
        break;
    }
}

if ($hasSeoNoindexParams) {
...
<?php /* SEO final robots cleanup on full page buffer */ ?>
    <?php
    if (!empty($hasSeoNoindexParams)) {
        AddEventHandler('main', 'OnEndBufferContent', static function (&$content) {
            ...
            $content = preg_replace(
                '~</head>~i',
                '    <meta name="robots" content="noindex, follow" />' . PHP_EOL . '</head>',
                $content,
                1
            );
        });
