		...
		<form method="post" id="one_click_buy_form" action="<?=$arResult['SCRIPT_PATH']?>/script.php" >
            ...
            <div class="form-header">
                ...
                <?foreach($arParams['PROPERTIES'] as $field):
                    ...
                    <div class="row" data-sid="<?=$field?>">
                        <div class="col-md-12">
                            <div class="form-group animated-labels">
                                <label for="POPUP_<?=$field?>">
                                    <?if($field == $field):?>
                                        <?=GetMessage('CAPTION_'.$field)?>
                                    <?else:?>
                                        <?=$arResult["PROPS"][$field]["TITLE"];?>
                                    <?endif;?>
                                    <?if (in_array($field, $arParams['REQUIRED'])):?><span class="required-star">*</span><?endif;?>
                                </label>
                                <?if($field=="PHONE"){
                                    $class.=" phone";
                                }?>
                                ...
                                <?if(in_array($field, $arParams['REQUIRED'])){
                                    $class.=" required";
                                }?>
                                <div class="input">
                                <?if($field=="COMMENT"):?>
                                    <textarea name="ONE_CLICK_BUY[<?=$field?>]" id="POPUP_<?=$field?>" class="<?=$class;?> form-control"></textarea>
                                <?else:?>
                                    ...
                                    <?else:?>
                                            ...
                                            <input type="<?=($field=="EMAIL" ? "email" : ($field=="PHONE" ? "tel" : "text"));?>" name="ONE_CLICK_BUY[<?=$field?>]" value="<?=$value?>" class="<?=$class;?> form-control" id="POPUP_<?=$field?>" />
                ...
                <div class="form-footer">
                    <?if($arParams["USE_CAPTCHA"] == "Y"):?>
                        <div class="captcha-row">
                            <label><span><?=GetMessage("CAPTCHA_LABEL")?>&nbsp;<span class="required-star">*</span></span></label>
                            <div class="captcha_image">
                                <?$code = htmlspecialcharsbx($APPLICATION->CaptchaGetCode())?>
                                <img src="/bitrix/tools/captcha.php?captcha_sid=<?=$code;?>" class="captcha_img" border="0" />
                                <input type="hidden" name="captcha_sid" class="captcha_sid" value="<?=$code;?>" />
                    ...
                    <div class="">
                        ...
                        <?endif;?>
                        <button class="btn btn-default btn-lg" type="submit" id="one_click_buy_form_button" name="one_click_buy_form_button" value="<?=GetMessage('ORDER_BUTTON_CAPTION')?>"><span><?=GetMessage("ORDER_BUTTON_CAPTION")?></span></button>
                        <!--/noindex-->
                    </div>
                </div>
		...
		if($(form).valid()){
			if($(form).find('input.error').length || $(form).find('textarea.error').length){
				...
				if(window.renderRecaptchaById && window.asproRecaptcha && window.asproRecaptcha.key){
					if(window.asproRecaptcha.params.recaptchaSize == 'invisible' && typeof grecaptcha != 'undefined' && /*arNextOptions.THEME.ONE_CLICK_BUY_CAPTCHA === 'Y'*/ false){
...
<?if(!$arParams['INLINE_FORM']):?>
	if(arPriorityOptions['THEME']['VALIDATE_PHONE_MASK']){
		$('#one_click_buy_id_PHONE').inputmask( "mask", { "mask": arPriorityOptions['THEME']['VALIDATE_PHONE_MASK'], "showMaskOnFocus": true } );
	}
<?endif;?>
</script>