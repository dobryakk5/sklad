	...
	$('table.table').each(function(){
		...
		if(first_td.length)
		{
			...
			_this.find('tbody tr td').each(function(i){
				...
				$('<div class="th-mobile">'+first_td[j].textContent+'</div>').prependTo($(this));
				j++;
			})
		}
	})

	$('a[data-toggle="tab"][href="#video"]').on('click', function(){
		$(window).resize();
	});
    ...
    var oneClickBuy = function (elementID, iblockID, that) {
        ...
        if(window.matchMedia('(min-width:100px)').matches)
        {
            if(!$(that).hasClass('clicked'))
            {
                ...
                var script = arPriorityOptions['SITE_DIR']+'ajax/one_click_buy.php?ELEMENT_ID='+elementID+'&IBLOCK_ID='+iblockID+'&ELEMENT_QUANTITY='+elementQuantity+'&OFFER_PROPS='+fill_prop.props;
                var name = $(that).data('name');

                if($(that).attr('disabled') != 'disabled'){

                    var trigger = ''; var arTriggerAttrs = {};
                    $.each($(that).get(0).attributes, function(index, attr){
                        var attrName = attr.nodeName;
                        var attrValue = $(that).attr(attrName);
                        trigger += '[' + attrName + '=\"' + attrValue + '\"]';
                        arTriggerAttrs[attrName] = attrValue;
    ...
    var oneClickBuyBasket = function () {
        ...
        if(window.matchMedia('(min-width:100px)').matches)
        {
            if(!$('.fast_order').hasClass('clicked'))
            {
                ...
                $('.'+name+'_frame').jqm({trigger: '.'+name+'_trigger', onHide: function(hash) { onHidejqm(name,hash) }, onLoad: function( hash ){ onLoadjqm( name, hash ); }, ajax: arPriorityOptions['SITE_DIR']+'ajax/one_click_buy_basket.php'});
                $('.'+name+'_trigger').click();
            }
        }
        else
...
$(document).on( 'click', '.to-cart:not(.read_more)', function(e){
    ...
    if(!th.hasClass('clicked'))
    {
        ...
        if(th.data("empty_props")=="N"){
            ...
            $.ajax({
                type:"POST",
                url:arPriorityOptions['SITE_DIR']+"ajax/item.php",
                data:fill_prop,
                dataType:"json",
                success:function(data){
